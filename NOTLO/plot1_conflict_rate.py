...
def mean_delay_no_notice(p_link):
    """Reactive stops dominate; worsens as comms degrade (more surprises)."""
    base_delay  = 38.0   # minutes — frequent unplanned stops
    extra       = (1 - p_link) * 22.0
    return base_delay + extra

def mean_delay_notlo(p_link):
    """
    ...
    """
    base_delay  = 18.0   # minutes — planned detour overhead
    extra       = (1 - p_link) ** 1.5 * 14.0
    return base_delay + extra

# ─── Monte Carlo sampling ─────────────────────────────────────────────────────
def run_scenario(conflict_prob_fn, p_link, n_runs=N_RUNS):
    """
    ...
    per_step_p = 1 - (1 - conflict_prob_fn(p_link)) ** (1 / (N_OPS * N_STEPS / 10))
...
for label, fn in regimes.items():
    ...
    for p in P_LINK_VALS:
        fr, sfr, oph, soph = run_scenario(fn, p)
    ...
    results[label] = {
        ...
        "delay":    np.array([
            (mean_delay_no_notice if "No Notice" in label else mean_delay_notlo)(p)
            for p in P_LINK_VALS
        ]),
    }
...
fig.subplots_adjust(top=0.82, bottom=0.13)   # room for suptitle above, x-labels below

ax_frac  = fig.add_subplot(gs[0])   # Metric A: fraction of runs
ax_oph   = fig.add_subplot(gs[1])   # Metric B: conflicts per operator-hour
ax_delay = fig.add_subplot(gs[2])   # Panel C: delay vs p_link (safety-vs-burden)
...
# Reduction callout at p_link = 1.0
...
# Shade the "NOTLO advantage" region between the two delay curves
...
fig.suptitle(
    ...
    r"($N_{ops}=8$,  1 000 Monte Carlo runs)",
    fontsize=12, fontweight="bold",
)

plt.savefig("plot1_conflict_rate.png", dpi=180, bbox_inches="tight")
print("Saved: plot1_conflict_rate.png")
plt.show()
